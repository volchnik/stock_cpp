...
void Generation::IterateGeneration(bool recalc) {
    ...
    if (recalc) {
        ...
        for (auto stategy : strategy_) {
            strategy_new.push_back(pair<std::shared_ptr<Operator>, double>(stategy.first, GetStrategyFitness(stategy.first, this->trader_)));
        }
    ...
    for (unsigned long ii = 0; ii < max(initialSize / 2.0, generation_count_ / 2.0); ii++) {
        ...
        for (unsigned long j = 0; j< initialSize; j++) {
            if (i != j) {
                distributionInit.push_back((1.0 + pow(this->strategy_.at(j).second - this->strategy_.at(this->strategy_.size() - 1).second, 2.0)) * log(2*Operator::LevenshteinDistance(this->strategy_.at(j).first, this->strategy_.at(i).first) + 1)
        ...
        if (((double)rand() / (double)RAND_MAX) > 0.75) {
            GetCrossPoint(childOperator1, this->max_operator_depth_ - 1) = this->GenerateRandom(1 + floor((double)rand() / (double)RAND_MAX * this->max_operator_depth_));
        }
        Operator::SimplifyOperator(childOperator1);
        
        if (((double)rand() / (double)RAND_MAX) > 0.75) {
            GetCrossPoint(childOperator2, this->max_operator_depth_ - 1) = this->GenerateRandom(1 + floor((double)rand() / (double)RAND_MAX * this->max_operator_depth_));
        }
        ...
        fitness_results.emplace_back(thread_pool.enqueue([childOperator2, local_trader]{ return std::pair<shared_ptr<Operator>, double>(childOperator2, GetStrategyFitness(childOperator2, local_trader)); }));
    }
...
double Generation::GetStrategyFitness(const std::shared_ptr<Operator>& strategy, const Trader& trader) {
    ...
    return trader.CalculateTradeFitness(std::get<1>(result), std::get<2>(result), std::get<5>(result), std::get<6>(result), strategy);
}
